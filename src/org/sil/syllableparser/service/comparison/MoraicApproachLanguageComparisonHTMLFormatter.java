 ...
 *
 */
public class MoraicApproachLanguageComparisonHTMLFormatter extends
		ApproachLanguageComparisonHTMLFormatter {

	MoraicApproachLanguageComparer muComparer;

	public MoraicApproachLanguageComparisonHTMLFormatter(MoraicApproachLanguageComparer comparer,
			Locale locale) {
		super(comparer, comparer.getMua1().getLanguageProject(), comparer.getMua2()
				.getLanguageProject(), locale);
		initialize(comparer, locale, LocalDateTime.now());
		this.muComparer = comparer;
	}
	...
	public String format() {
		...
		formatNaturalClasses(sb, muComparer.getCVNaturalClassesWhichDiffer());
		formatTemplates(sb);
	...
	protected void formatSonorityHierarchy(StringBuilder sb) {
		...
		SortedSet<DifferentSHNaturalClass> diffNaturalCLasses = muComparer
	...
	protected void formatSonorityHierarchyOrder(StringBuilder sb) {
		LinkedList<Diff> diffs = muComparer.getSonorityHierarchyOrderDifferences();
		if (diffs.size() > 1) {
	...
	protected void formatTemplates(StringBuilder sb) {
		...
		SortedSet<DifferentTemplate> diffTemplates = muComparer
	...
	protected void formatTemplateOrder(StringBuilder sb) {
		LinkedList<Diff> diffs = muComparer.getTemplateOrderDifferences();
		if (diffs.size() > 1) {
			...
			sb.append(getAdjectivalForm("report.second", "report.adjectivalendingm"));
			sb.append("</th>\n</tr>\n</thead>\n<tbody>\n");
			List<Template> templates1 = muComparer.getMua1().getLanguageProject().getActiveAndValidTemplates();
			List<Template> templates2 = muComparer.getMua2().getLanguageProject().getActiveAndValidTemplates();
	...
	protected void formatFilters(StringBuilder sb) {
		...
		SortedSet<DifferentFilter> diffFilters = muComparer
	...
	protected void formatFilterOrder(StringBuilder sb) {
		LinkedList<Diff> diffs = muComparer.getFilterOrderDifferences();
		if (diffs.size() > 1) {
			...
			sb.append(getAdjectivalForm("report.second", "report.adjectivalendingm"));
			sb.append("</th>\n</tr>\n</thead>\n<tbody>\n");
			List<Filter> sonorityHierarchy1 = muComparer.getMua1().getLanguageProject().getActiveAndValidFilters();
			List<Filter> sonorityHierarchy2 = muComparer.getMua2().getLanguageProject().getActiveAndValidFilters();
			int size1 = sonorityHierarchy1.size();
			int size2 = sonorityHierarchy2.size();
			int maxSize = Math.max(size1, size2);
			for (int i = 0; i < maxSize; i++) {
